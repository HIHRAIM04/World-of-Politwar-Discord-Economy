...
module.exports = {
	name: 'work',
	description: 'Команда для получения ежедневного бонуса (100 долларов).',
    category: 'economy',
	execute(message, args) {
        ...
        message.channel.send(`Возвращайтесь через ${timeH} ${checkHours(timeH)}`)
            ...
            } else if(!(users[message.author.id].dollars || users[message.author.id].bonus || (users[message.author.id].dollars && users[message.author.id].bonus))){
                users[message.author.id].dollars = bonus
            ...
            let bonusMess = new Discord.MessageEmbed()
                .setTitle('Ежедневный бонус долларов')
                .setDescription(`Собрано ${bonus} :dollar:`)
