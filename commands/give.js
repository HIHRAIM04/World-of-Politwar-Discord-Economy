...
module.exports = {
	name: 'give',
	description: 'Команда для передачи денег или предмета другому участнику. Чтоб передать деньги другому участнику, используйте: `wop!give money "Пинг участника" "Кол-во денег"`. Чтоб передать предмет из инвентаря другому участнику, используйте: `wop!give item "Пинг участника" "Номер предмета в магазине"`',
    category: 'economy',
	execute(message, args) {
        ...
        if(args[0] == 'money') {
            ...
            if(parseInt(args[2]) > users[message.author.id].dollars || !users[message.author.id || parseInt(args[2]) < 1]) return message.channel.send({content: "У вас недостаточно долларов для совершения операции!"})
        ...
        function giveToWallet(id, dollars) {
            users[message.author.id].dollars -= dollars
            users[id].dollars += dollars
            fs.writeFile("./users.json", JSON.stringify(users), (err) => { // Всё сохраняется в .json файле
                if (err) console.log(err)
            });
        }
