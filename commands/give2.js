...
module.exports = {
	name: 'give2',
	description: 'Команда для передачи денег или предмета другому участнику. Чтоб передать деньги другому участнику, используйте: `wop!give2 money "Пинг участника" "Кол-во денег"`. Чтоб передать предмет из инвентаря другому участнику, используйте: `wop!give2 item "Пинг участника" "Номер предмета в магазине"`',
    category: 'economy2',
	execute(message, args) {
        ...
        if(args[0] == 'money') {
            if(!args[2] || parseInt(args[2]) < 1 || isNaN(parseInt(args[2]))) return message.channel.send({content: "Укажите количество фунтов, которое вы хотите передать!"})
            if(parseInt(args[2]) > users[message.author.id].pounds || !users[message.author.id || parseInt(args[2]) < 1]) return message.channel.send({content: "У вас недостаточно фунтов для совершения операции!"})
            giveToWallet(argsUser.id, parseInt(args[2]))
            let giveMess = new Discord.MessageEmbed()
                .setDescription(`${args[2]} :dollar: передано в кошелёк ${argsUser}`)
                .setColor('00ff00')
        ...
        function giveToWallet(id, pounds) {
            users[message.author.id].pounds -= pounds
            users[id].pounds += pounds
            fs.writeFile("./users.json", JSON.stringify(users), (err) => { // Всё сохраняется в .json файле
                if (err) console.log(err)
            });
        }
